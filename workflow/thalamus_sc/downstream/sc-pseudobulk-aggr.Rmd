...
factor_groups <- list(
    general_disease=c('Control-FTD', 'Control-AD', 'FTD', 'AD'),
...
replace_dispersion_function <- TRUE
```
...
acontrol_rows <- sampletable[[disease_col]] == "Control" &
    str_detect(sampletable[[subset_col]], "Mathys")
fcontrol_rows <- sampletable[[disease_col]] == "Control" &
    ! str_detect(sampletable[[subset_col]], "Mathys")
sampletable[[disease_col]] <- as.character(sampletable[[disease_col]])
sampletable[acontrol_rows, disease_col] <- "Control-AD"
sampletable[fcontrol_rows, disease_col] <- "Control-FTD"
...
Log-normalized pseudobulk counts are compared between disease conditions across the datasets. P-values
and FDR values were calculated using the two-tailed Wilcoxon Rank Sum test and Benjamini-Hochberg (BH)
method, respectively. Junctions with zero counts across all samples are excluded.


```{r prep_counting}

# --------------------------------------------------------------------------------
# This chunk compares junction counting
# --------------------------------------------------------------------------------

norm_list <- mclapply(names(res_list), function(name) {
...
set_names(names(res_list))

# Function to calculate p-values using t-test
        ...
        tryCatch(
            { 
                if (stat == "ttest") {
                    ...
                    wilcox.test(as.formula(formula_str), data=df_i)$p.value
                } else {
...
```{r junction_comparisons, results='asis', fig.width=10}

# --------------------------------------------------------------------------------
# This chunk compares junction counts
# --------------------------------------------------------------------------------
...
for (name in names(res_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            cat('#### Comparison\n\n')
            # Plot
            p <- ggplot(ce_df, aes_string(x=disease_col, y='counts', fill=disease_col)) +
                geom_violin(trim=FALSE) +
                geom_boxplot(fill="white", width=0.1) +
                theme_bw() +
                facet_wrap(. ~ junction,
                           scales="free_y") +
                           # labeller=labeller(junction=label_wrap_gen(width=15, multi_line=TRUE))) +
            ...
            # Statistical analysis using wilcoxon rank sum test on differences
            pval_vec <- calc_pval(dataframe=ce_df,
                                  response_col="counts",
                                  stat="wilcox",
                                  explanatory_col=disease_col,
                                  split_col="junction")

            pval_df <- data.frame(junction=names(pval_vec),
                                  pvalue=pval_vec,
            ...
            stat_df <- split(ce_df, df$junction) %>%
            ...
            cat('\n\n')
            # Save the full stat table
            csv_path <- paste0(out_dir, "/", name, "_", ce, "_junction_counts.csv")
            write.csv(ce_df,
                      csv_path,
                      row.names=FALSE,
                      quote=FALSE)
...
Log-normalized pseudobulk counts are compared between disease conditions across the datasets.
P-values and FDR values were calculated using the two-tailed Wilcoxon Rank Sum test and BH
method, respectively. Junctions with zero counts across all samples are excluded.

```{r ce_comparisons, results='asis', fig.width=8}

# --------------------------------------------------------------------------------
# This chunk compares CE counts
# --------------------------------------------------------------------------------
...
for (name in names(res_list)) {
    ...
    for (ce in ce_targets) {
        ...
        # Subset rows for the corresponding CE target gene
        ce_df <- df[str_detect(df$junction, ce), ] %>%
            group_by_at(vars(sample_col, "exon_detection", disease_col)) %>%
        ...
        if (nrow(ce_df) > 0) {
            ...
            p <- ggplot(ce_df, aes_string(x=disease_col, y='counts', fill=disease_col)) +
                ...
                geom_boxplot(fill="white", width=0.1) +
                theme_bw() +
                facet_wrap(. ~ exon_detection, scales="free_y") +
                ylab('log-normalized counts for junctions with or without CEs') +
                theme(axis.title.x=element_blank(),
                      axis.text.x=element_text(angle=90, vjust=0.5, hjust=1)) +
                scale_fill_manual(values=color_map[[disease_col]])
            ...
            pval_df <- data.frame(exon_detected=names(pval_vec),
                                  pvalue=pval_vec,
                                  FDR=p.adjust(pval_vec, method="BH")) %>%
                remove_rownames()
            ...
            cat('#### Counts\n\n')
            # Display mean and median counts for each junction
            stat_df <- split(ce_df, df$exon_detection) %>%
                map_df(function(df_i) df_i %>%
                       group_by_at(vars(disease_col, 'exon_detection')) %>%
                       summarize(exon_detection=exon_detection,
                                 N=n(),
                                 average_counts=mean(counts),
                                 median_counts=median(counts)) %>%
                       unique()) %>%
                as.data.frame() %>%
                arrange(exon_detection)
            print(knitr::kable(stat_df))
            cat('\n\n')
            # Save the full stat table
            csv_path <- paste0(out_dir, "/", name, "_", ce, "_CE_counts.csv")
            write.csv(ce_df,
                      csv_path,
                      row.names=FALSE,
                      quote=FALSE)
...
(e.g. STMN2) within a subset (e.g. Mathys_ExNeu1).


```{r ce_ratio, results='asis', fig.width=6}

# --------------------------------------------------------------------------------
# This chunk compares CE/non-CE counts
# --------------------------------------------------------------------------------
...
for (name in names(res_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (length(e_detection) > 1 & sum(ce_df$counts) > 0) {
            ...
            cat('\n\n#### Comparison\n\n')
            # Plot
            p <- ggplot(ce_df, aes_string(x=disease_col, y='ratio', fill=disease_col)) +
                geom_violin(trim=FALSE) +
                geom_boxplot(fill="white", width=0.1) +
                theme_bw() +
                ylab('Count ratios (CE / non-CE)') +
                theme(axis.title.x=element_blank(),
                      axis.text.x=element_text(angle=90, vjust=0.5, hjust=1)) +
                scale_fill_manual(values=color_map[[disease_col]]) +
                scale_y_log10()
            ...
            # Statistical analysis using t-test on differences
            stat_label <- "wilcox"
            pval_vec <- calc_pval(dataframe=ce_df,
                                  response_col="ratio",
                                  explanatory_col=disease_col,
            ...
            cat(paste0('\n\n- P-value: ', pval_vec, ' (two-tailed ', stat_label, ')\n\n'))
            ...
            # Save the full stat table
            csv_path <- paste0(out_dir, "/", name, "_", ce, "_CE_over_nonCE.csv")
            write.csv(ce_df,
                      csv_path,
                      row.names=FALSE,
                      quote=FALSE)
...
for (name in names(res_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (length(e_detection) > 1 & sum(ce_df$counts) > 0) {
            ...
            cat('\n\n')
            # Save the full stat table
            csv_path <- paste0(out_dir, "/", name, "_", ce, "_CE_over_all.csv")
            write.csv(ce_df,
                      csv_path,
                      row.names=FALSE,
                      quote=FALSE)
