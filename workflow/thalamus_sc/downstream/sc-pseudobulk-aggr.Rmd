...
# Compare size factors vs sum factors
for (name in names(dds_list)) {
    sf_df <- data.frame(pos=sizeFactors(dds_list[[name]]),
                        sum=sizeFactors(scr_list[[name]])) %>%
        rownames_to_column(sample_col) %>%
        left_join(coldata_list[[name]], by=sample_col)
    print("##", name, "\n\n")
    print(knitr::kable(head(sf_df)))
    print("\n\n")
}
```
