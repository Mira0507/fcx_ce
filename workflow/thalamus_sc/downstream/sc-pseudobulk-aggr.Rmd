...
Log1p-normalized pseudobulk counts are compared between disease conditions across the datasets. P-values
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p <- ggplot(ce_df, aes_string(x=disease_col, y='counts', fill=disease_col)) +
                ...
                ylab('log1p-normalized junction counts') +
...
Log1p-normalized pseudobulk counts are compared between disease conditions across the datasets.
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p <- ggplot(ce_df, aes_string(x=disease_col, y='counts', fill=disease_col)) +
                ...
                ylab('log1p-normalized counts for junctions with or without CEs') +
                theme(axis.title.x=element_blank(),
                      axis.text.x=element_text(angle=90, vjust=0.5, hjust=1)) +
                scale_fill_manual(values=color_map[[disease_col]])
