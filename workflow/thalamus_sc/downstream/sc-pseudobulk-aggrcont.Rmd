...
Log-normalized pseudobulk counts are compared across all disease conditions. Junctions 
...
for (name in names(norm_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p <- ggplot(ce_df,
                        aes_string(x=disease_col, y='counts', fill=disease_col)) +
                geom_violin(trim=FALSE) +
                theme_bw() +
                geom_point(data=ce_df,
                           aes_string(x=disease_col, y='counts', color=study_col)) +
...
Junctions are counted based on whether it results in CE generation.
    ...
    for (ce in ce_targets) {
        ...
        if (length(e_detection) > 1 & sum(ce_df$counts) > 0) {
            ...
            p <- ggplot(ce_df, aes_string(x=disease_col, y='ratio', fill=disease_col)) +
                geom_violin(trim=FALSE) +
                geom_point(data=ce_df,
                           aes_string(x=disease_col, y='ratio', color=study_col)) +
                stat_summary(fun="median",
                             geom="crossbar",
                             width=0.1,
                             color="black") +
...
# Pairwise differential testing using Wilcoxon Rank Sum test {.tabset}
