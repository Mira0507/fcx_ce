...
# Differential testing (junction) {.tabset}
...
```{r binarize_matrix_junction}

# --------------------------------------------------------------------------------
# This chunk performs fisher's exact test after binarizing the count matrices
# --------------------------------------------------------------------------------

# Binarize each count matrix (1 for detected, 0 for undetected)
b_list <- mclapply(mat_list, function(m) ifelse(m > 0, 1, 0))

# Specify subsets to be analyzed
pairwise_subsets <- names(b_list)[!str_detect(names(b_list), "all")]

res_list <- lapply(pairwise_subsets, function(name) {
    ...
    stats_df <- mclapply(1:nrow(mat), function(i) {
        # Subset a row
        mat_i <- mat[i, ]
    ...
    return(f_df) }) %>%
    # Name the list
    set_names(pairwise_subsets)
```

Metrics:

- **subset**: study and celltype
- **junction**: `<chr>:<start>:<end>:<gene_name>:<junction_category>`
- **N**: the number of samples for each condition
- **Conditions (e.g. Control, FTD, AD)**: detection rate within each condition
- **relative_detection**: relative detection (detection rate in disease / detection rate in control)
- **pvalue**: raw p-values
- **FDR**: false discovery rate (FDR) computed using the Benjamini-Hochberg (BH) method

```{r junction_de_full, results='asis'}

# --------------------------------------------------------------------------------
# This chunk displays full results for per-junction fisher's exact test
# --------------------------------------------------------------------------------
...
for (name in names(res_list)) {
    ...
    # Save
    csv_path <- file.path(out_dir,
                          paste0(name, '_differential_results_junction.csv'))
...
# Differential testing (CE) {.tabset}

```{r binarize_matrix_ce}
