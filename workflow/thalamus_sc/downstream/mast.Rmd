...
for celltype in adata_dic.keys():
    ...
    ann_new = ad.AnnData(X=counts_subset.T, obs=ann.obs.loc[counts_subset.columns, :])
...
The following table shows the number of barcodes for all cells (`all`) or
celltype-specific subsets (`ExNeu1` and `ExNeu2`). The `N_barcodes` column represents the number of 
barcodes that were initially loaded.
...
n_barcodes <- sapply(py$adata_dic, function(o) o$shape[[1]])
df <- data.frame(cells=names(sca_list),
                 N_barcodes=c(sum(n_barcodes), n_barcodes)) %>%
    remove_rownames()
...
df[['N_remaining']] <- sapply(sca_list, ncol)
...
for (subset in names(sca_list)) {
    ...
    # Create boxplots
    for (countype in names(mat_list)) {
        cat('####', countype, '{.tabset}\n\n')
        for (condition in names(factor_groups)) {
            cat('#####', condition, '\n\n')
            df <- mat_list[[countype]][, c('barcode', 'counts', condition)]
            p <- ggplot(df, aes_string(x=condition,
                                       y='counts',
                                       fill=condition)) +
                theme_bw() +
