---
title: "Single cell differential splicing (DS) analysis"
author: "Mira Sohn"
output:
    html_document:
        code_folding: hide
...
Sample-to-sample similarity is explored using a heatmap generated from non-zero cells, 
...
for (name in names(sp_list)) {
    cat('###', name, '{.tabset}\n\n')
    # Retrieve log2(count + 1) matrix
    dmat <- as.matrix(dist(t(log2(sp_list[[name]] + 1))))
    ...
    heatmap_meta <- heatmap_meta[, c(sample_col, condition_cols, 'SampleID_celltype')] %>%
        remove_rownames() %>%
        unique()
    rownames(heatmap_meta) <- heatmap_meta[['SampleID_celltype']]
    ...
    print(pheatmap::pheatmap(
        ...
        scale='none',
        color=colors,
        annotation_col=heatmap_meta[colnames(dmat), condition_cols],
        show_rownames=FALSE,
        show_colnames=FALSE,
        cluster_cols=TRUE,
        border_color=NA
...
for (name in names(sp_list)) {
    cat('###', name, '{.tabset}\n\n')
    # Retrieve log2(count + 1) matrix
    dmat <- as.matrix((log2(sp_list[[name]] + 1)))
