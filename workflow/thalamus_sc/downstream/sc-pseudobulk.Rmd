...
and FDR values were calculated using the two-tailed t-test and BH method.
...
# Prep a list of tables consisting of normalized counts and metadata
norm_list <- mclapply(names(res_list), function(name) {

    df <- obj[['bulk']][[name]][['norm']] %>%
...
(e.g. STMN2) within a subset (e.g. Marsan_ExNeu1).
    ...
    for (ce in ce_targets) {
        ...
        if (length(e_detection) > 1 & sum(ce_df$counts) > 0) {
            ...
            p <- ggplot(ce_df, aes_string(x=disease_col, y='ratio', fill=disease_col)) +
                geom_boxplot(color="black", width=0.5) +
                theme_bw() +
                ylab('Junction counts for CE / non-CE') +
                theme(axis.title.x=element_blank(),
                      axis.text.x=element_text(angle=90, vjust=0.5, hjust=1))
