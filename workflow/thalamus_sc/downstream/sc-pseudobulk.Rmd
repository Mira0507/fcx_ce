...
It's assumed that only the *cryptic* type generates CEs by starting or ending at intron.
...
# BED files for individual splice junctions

The BED files generated here contain the splice junctions tested in the current differential analysis.
They are prepared for data exploration using IGV. Refer to the naming convention when loaded on IGV: 
`<chromosome>:<start>:<end>:<gene_name>:<exon_type>`. 


```{r create_intron_bed, eval=FALSE}
