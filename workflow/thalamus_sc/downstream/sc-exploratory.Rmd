...
for (name in names(sp_list)) {
    ...
    # Prep metadata
    m_df <- as.data.frame(py$subadata_dic[[name]][['obs']])
    condition_cols <- names(factor_groups)[names(factor_groups) %in% colnames(m_df)]
    m_df <- m_df[, condition_cols] %>%
