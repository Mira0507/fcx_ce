...
exon_levels <- list(factors=c('junction undetected',
...
for (celltype in names(py$adata_dic)) {
    ...
    for (gene in genes) {
        ...
        e_cols <- colnames(gdf)[-1]
        for (i in 1:nrow(gdf)) {
            e_code <- ""
            for (c in c('cryptic', 'non-cryptic-annotated', 'non-cryptic-novel')) {
                if (c %in% e_cols) {
                    new_code <- ifelse(gdf[i, c] > 0, c, "")
                    e_code <- paste0(e_code, " ", new_code)
                    gdf[i, 'temp_code'] <- e_code
                }
            }
        }
        gdf[['exon_detection']] <- ifelse(str_detect(gdf[['temp_code']], " cryptic "),
                                          "CE detected",
                                   ifelse(str_detect(gdf[['temp_code']], "non-cryptic"),
                                          "junction detected without CE",
                                          "junction undetected"))
        ...
        for (name in names(gdf_list)) {
            cat("######", name, "\n\n")
            # Print UMAP
            p <- ggplot(gdf_list[[name]], aes(x=UMAP1, y=UMAP2, color=exon_detection)) +
                geom_point(size=0.5) +
                theme_bw() +
