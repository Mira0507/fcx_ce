...
# Clean the rownames
rownames(all_meta) <- paste(
    all_meta[['cell_barcode']],
    all_meta[['SampleID']],
    sep="_")
...
for subs in r.n_df['subset'][1:]:
    ...
    # Retrieve the AnnData obj for all cells
    all_adata = subadata_dic['all']
    # Add subsetted AnnData to the dictionary
    subadata_dic[subs] = all_adata[all_adata.obs[key_col] == target]
...
PCA is omitted if a subset includes too few cells to reduce dimensions.
...
# --------------------------------------------------------------------------------
# This chunk specifies variables and a function required to generate exploratory plots
# --------------------------------------------------------------------------------
...
## Cell number bias {.tabset}

We analyze the association between per-sample cell number and the following metrics:
...
Individual linear regression models (e.g. `junction_counts ~ sample_cells` or
$junction\_counts = sample\_cells \times \beta_1 + \beta_0$) were fitted to determine 
how significantly each metric (e.g. `junction_counts`) was affected by the per-sample 
cell number for each exon type. The `parameter` and `p_value` columns represent $\beta_1$,
the magnitude and direction of change, and the likelihood of rejecting
the null hypothesis, respectively. Additionally, the magnitude of correlation
between two variables was summarized using an R-squared value, represented by the `R_squared` column.

Ideally, we'd like to use metrics that are unbiased by per-sample cell number when examining per-cell 
CE counts across samples and disease conditions.

The following table summarizes the p-values calculated using a two-tailed Wilcoxon Rank Sum test
on **`percell_junctions`** and **`percent_detection`** comparing each disease to Control. 
...
```{r ncell_validation, results='asis', fig.width=6, fig.height=4}

# --------------------------------------------------------------------------------
# This chunk analyzes the relationship between per-sample cell number and various CE metrics
# --------------------------------------------------------------------------------
    ...
    for (counttype in names(ncount_list[[celltype]])) {
        ...
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            ...
            for (m in y_metric) {
                cat('\n\n######', m, '\n\n')
                p <- ggplot(input_df,
                            aes_string(x='sample_cells',
                                       y=m,
                                       color='general_disease',
                                       shape='exon_type')) +
                    geom_point(alpha=0.7, size=3) +
                    theme_bw() +
                ...
                # Run statistics
                design <- paste(m, '~ sample_cells')
                model_list <- split(input_df, input_df[['exon_type']]) %>%
                    map(~lm(as.formula(design), data=.x) %>% summary)

                cat(paste0('\n\n- Model fitted: **', design, '**\n\n'))
                summary_df <- data.frame(
                    exon_type=names(model_list),
                    R_squared=map(model_list, ~.x$r.squared) %>% unlist,
                    parameter=map(model_list, ~.x$coefficients[,1]) %>%
                        map(~.x[2]) %>%
                        unlist,
                    p_value=map(model_list, ~.x$coefficients[,4]) %>%
                        map(~.x[2]) %>% 
                        unlist) %>%
                remove_rownames()
...
## Plots and tables {.tabset}

```{r ncell_plot_prep}

# --------------------------------------------------------------------------------
# This chunk prepares plotting to visualize per-cell junction metrics
# --------------------------------------------------------------------------------

# Specify per-cell metrics to be plotted along with the axis title
percell_metrics <- list(
    percell_junctions='Number of Junctions Detected per Cell',
    percent_detection='% Cells with Junction Detection'
)

# Function to generate a volcano plot for per-cell CE metrics
plot_percell_junctions <- function(ycol, ylabel) {
    ggplot(input_df,
        ...
        geom_quasirandom(
            data=input_df,
            aes_string(x=disease_col, y=ycol, color=study_col),
            width=0.2, size=3) +
        stat_summary(fun="median",
             ...
             color="black") +
        ylab(ylabel) +
        facet_wrap(~exon_type, scales="free_y") +
        theme(axis.title.x=element_blank(),
        axis.text.x=element_text(angle=90, vjust=0.5, hjust=1)) +
        scale_fill_manual(values=color_map[[disease_col]]) +
        scale_color_manual(values=color_map[[study_col]]) +
        ylim(min_y, max_y + ymax_added * max_y)
}

# Function to run a wilcoxon rank sum test on a specified metric
disease_pval <- function(dataframe=i_df,
                         diseases=disease_contrasts,
                         test_metric) {
    # Calculate p-values
    map_dbl(diseases, function(d) { 
        res <- wilcox.test(
            x=dataframe[dataframe[[disease_col]] == d, test_metric],
            y=dataframe[dataframe[[disease_col]] == 'Control', test_metric])
        return(res$p.value) 
    }) %>% 
...
# --------------------------------------------------------------------------------
# This chunk plots per-cell junction metrics
# --------------------------------------------------------------------------------
...
for (celltype in names(ncount_list)) {
    ...
    for (counttype in names(ncount_list[[celltype]])) {
        ...
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            ...
            stat_df_list <- list()
            # Plot
            for (y_var in names(percell_metrics)) {
                cat('\n\n######', y_var, 'plot\n\n')
                # Calculate the min and max values for the y-variable
                min_y <- min(input_df[[y_var]])
                max_y <- max(input_df[[y_var]])

                # Plot
                p <- plot_percell_junctions(
                    ycol=y_var,
                    ylabel=percell_metrics[[y_var]])
                print(p)
                cat('\n\n')
                p_path <- file.path(
                    out_dir,
                    paste0(celltype, '_', counttype, '_', gene, '_', y_var, '_violin.pdf')
                    )
                ...
                stat_df <- map(names(df_list),
                               ~df_list[[.x]] %>% mutate(exon_type=.x)
                    ) %>%
                    bind_rows() %>%
                    remove_rownames()
                stat_df_list[[y_var]] <- stat_df
            }

            cat('\n\n###### Summary table\n\n')
            stat_all_df <- stat_df_list %>% bind_rows()
            print(knitr::kable(stat_all_df))
            cat('\n\n')
            csv_summary <- file.path(out_dir, paste0(name, "_summary.csv"))
            write.csv(stat_all_df, csv_summary, row.names=FALSE, quote=FALSE)
...
# Samples with and without CEs {.tabset}

The proportion and number of samples with and without CEs are summarized as a barplot 
across diseases and genes. 

```{r sample_ce_detection, results='asis', fig.width=6, fig.height=4}

ce_colname <- c('detected', 'undetected')
for (celltype in names(ncount_list)) {
    cat('\n\n##', celltype, '{.tabset}\n\n')
    for (counttype in names(ncount_list[[celltype]])) {
        cat('\n\n###', counttype, '{.tabset}\n\n')
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            ...
            input_df <- ncount_list[[celltype]][[counttype]][[gene]] %>%
                dplyr::select(
                    ...
                    -percent_detection,
                    -study) %>%
                pivot_wider(names_from=exon_type, values_from=junction_cells) %>%
                mutate(CE=ifelse(cryptic > 0, "detected", "undetected")) %>%
                group_by_at(vars(disease_col, 'CE')) %>%
                summarize(N=n())

            # Add columns for absent detection categories
            ce_category <- unique(input_df[['CE']])
            for (ce_i in ce_colname) {
                if (!ce_i %in% ce_category) {
                    placeholder_df <- input_df
                    placeholder_df[['CE']] <- rep(ce_i, nrow(placeholder_df))
                    placeholder_df[['N']] <- rep(0, nrow(placeholder_df))
                    input_df <- rbind(input_df, placeholder_df)
                } 
            }

            # Plot
            for (bartype in c('fill', 'stack')) {

                ylabel <- ifelse(bartype == "fill",
                                 "Sample Proportion",
                                 "Sample Count")

                cat('\n\n#####', ylabel, "\n\n")
                p <- ggplot(input_df,
                            aes_string(x=disease_col, y='N', fill='CE')) +
                    geom_bar(width=0.5, position=bartype, stat='identity') +
                    theme_bw() +
                    ylab(ylabel) +
                    theme(axis.title.x=element_blank())
                print(p)
                ...
                plot_path <- file.path(
                    out_dir,
                    paste0(celltype, "_", counttype, "_", gene, "_", bartype, "_barplot.pdf")
                )
                ggsave(plot_path, p, device="pdf")
                link_plot(plot_path)
            }
            cat('\n\n##### Table\n\n')
            print(knitr::kable(input_df %>% arrange(CE)))
        }
    }
}
```
...
rds <- list(
    ...
    bulk=sp_list,
    meta=do.call("rbind", 
                 lapply(py$subadata_dic, function(o) as.data.frame(o[['obs']]) %>% 
                        rownames_to_column('barcodes'))
    )
)
