...
# Specify target CE
tdp_target <- 'chr8:79611214:79616822:STMN2:cryptic'
...
# Specify Sporadic FTD sample IDs
sftd_ids <- c('4571-T', '6562-T', '6400-T')
```
...
sampletable[[ftd_col]] <- ifelse(
    sampletable[[id_col]] %in% sftd_ids,
    "Sporadic",
    "GRN"
)
...
for (name in names(l2t_list)) {
    ...
    cat("Not enough dimensionality for PCA...")
    cat('\n\n')
}
```
...
norm_list <- mclapply(names(mat_list), function(name) {
    ...
    # Add a column for the association with the TDP43 function for each junction
    df[['CE_generation']] <- ifelse(
        str_detect(df[['junction']], tdp_target),
        "TDP43 dysfunction", ifelse(
            str_detect(df[['junction']], "\\:cryptic"),
            "Unknown",
            "Not CE"
        )
    )
...
for (name in names(norm_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p_path <- file.path(
                out_dir,
                paste0(lab, '_', ce, '_individual_junctions.svg'))
            ggsave(p_path, p, device="svg", width=w, height=h)
            link_plot(p_path)
        } else {
            cat("\n\nCount not displayed!\n\n")
        }
...
for (name in names(norm_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p_path <- file.path(
                out_dir,
                paste0(lab, '_', ce, '_junction_categories.svg'))
...
The counts for junctions with TDP43-affected CEs are divided by the counts for all extracted junctions. 
Note that samples not containing any junctions are omitted. 1/10 of non-zero minimum value is added 
to all remaining entries to avoid errors when dividing values. The non-zero minimum value was calculated
for each CE target (e.g. STMN2) within a subset (e.g. ExNeu SOX14-).
...
for (name in names(mat_list)) {
    ...
    for (ce in ce_targets) {
        ...
        e_detection <- unique(ce_df$CE_generation)
        if (length(e_detection) > 1 & sum(ce_df$counts) > 0) {
            ...
            ce_df <- ce_df %>%
                pivot_wider(names_from=CE_generation, values_from=counts) %>%
            ...
            ce_df <- ce_df[ce_df[['all junctions']] > 0,] %>%
                ...
                mutate(ratio=`TDP43 dysfunction` / `all junctions`,
                       metric="Ratio") %>%
                as.data.frame() %>%
                unique()
...
for (name in names(ratio_list)) {
    ...
    if (!is.null(ce_df)) {
        ...
        p <- ratio_plot(input_df=ce_df,
                        ...
                        scol=study_col,
                        fcol=ftd_col,
                        ylabel='Count Ratios (TDP43-affected CE / All-junctions)') + 
        ...
        p_path <- file.path(
            out_dir,
            paste0(lab, '_tdpce_over_alljunction.svg'))
        ggsave(p_path, p, device="svg")
        link_plot(p_path)
        cat('\n\n')

        # Save and link the input table
        t_path <- file.path(
            out_dir,
            paste0(lab, '_tdpce_over_alljunction.tsv'))
        write.table(
            ce_df,
            t_path,
            sep="\t",
...
for (name in names(norm_list)) {
    ...
    for (d in diseases) {
        ...
        if (d %in% df[[disease_col]]) {
            # Subset the input data frame 
            df_subset <- subset_clean_counts(
                dataframe=df,
                disease_label=d,
                split_col="CE_generation")
            ...
            stat_df <- map(names(stat_list), ~stat_list[[.x]] %>%
                group_by_at(vars(disease_col, 'CE_generation')) %>%
                summarize(gene=.x,
                          N=n(),
                          average_counts=mean(counts),
                          median_counts=median(counts)) %>%
            ...
            res_df <- inner_join(pval_df,
                                 stat_df,
                                 by=c('gene', 'junction'='CE_generation')) %>%
                clean_pval(dcol=disease_col)
            # Clean column order
...
```{r wilcox_ratios_ce_all, results='asis', eval=FALSE}
...
The following BED file contains exons extracted from the following input GTF file:

```{r gtf_path}
print(gtf_path)
```

```{r extract_exons, results='asis'}

# --------------------------------------------------------------------------------
# This chunk extracts exons from the input GTF file
# --------------------------------------------------------------------------------

# Import the input GTF file and filter exons for genes of interest
gtf <- import(gtf_path) %>%
    subset(type == "exon" & gene_name %in% ce_targets) %>%
    as.data.frame() %>%
...
# Save and link
gtf_out <- file.path(out_dir, "exons.bed")
write.table(
    gtf, 
    gtf_out,
    sep="\t",
