...
mtx_path <- "../results/junction_counts/thalamus_inhibitory_perind_numers.counts.gz"
...
subset_col <- list(study_celltype=c(
    'study',
    'renamed_annotation_cell_type_post_outlier_removal')
)
...
# Number of cells and CEs {.tabset}

The number of single cells with and without CEs along with the number of CEs per dataset are 
counted across the datasets. Both raw and normalized counts are displayed as shown below.


```{r prop_table}

# --------------------------------------------------------------------------------
# This chunk assesses the number of CEs and cells with CEs
# --------------------------------------------------------------------------------
...
for (celltype in names(py$adata_dic)) {
    ...
    o <- py$adata_dic[[celltype]]
    m_df <- o[['obs']][, c(names(subset_col)[1], sample_col, disease_col)] %>%
        rownames_to_column('barcodes')
    ncell_list[[celltype]] <- list()
    for(counttype in c('junction_raw', 'junction_lognorm')) {
        df <- o[['obsm']][[counttype]] %>%
            as.data.frame() %>%
            rownames_to_column('barcodes') %>%
            ...
            as.data.frame()
        df_list <- split(df, df$junction) %>%
            mclapply(function(df_i) {

                # Calculate the number of samples for each subset
                ncell_df <- df_i[, c(names(subset_col)[1], disease_col, sample_col)] %>%
                    unique() %>%
                    group_by_at(vars(names(subset_col)[1], disease_col)) %>%
                    summarize(subset_samples=n()) %>%
                    # Add columns for the number of cells in input and the subset
                    full_join(
                        df_i[, c("barcodes", names(subset_col)[1], disease_col)] %>%
                            unique() %>%
                            mutate(all_cells=length(unique(df_i$barcodes))) %>%
                            group_by_at(vars(names(subset_col)[1], disease_col)) %>%
                            summarize(all_cells=all_cells, subset_cells=n()) %>%
                            unique(),
                        by=c(names(subset_col)[1], disease_col))

                # Filter cells containing CEs
                subcell_df <- df_i %>%
                    dplyr::filter(exon_type == "cryptic" & counts > 0)

                # Next junction if none of the barcodes remain
                if (nrow(subcell_df) == 0) { 
                    return(NULL)
                }

                # Count the number of cells containing CEs and CEs per subset
                subcell_df <- subcell_df %>%
                    group_by_at(vars(names(subset_col)[1], disease_col)) %>%
                    summarize(CE_cells=n(), CE_counts=sum(counts)) %>%
                    unique()

                # Consolidate all counts
                ncell_df <- full_join(ncell_df,
                                      subcell_df,
                                      by=c(names(subset_col)[1], disease_col)) %>%
                    as.data.frame() %>%
                    mutate(CEcounts_per_cell=CE_counts / subset_cells,
                           percent_CEcells=CE_cells / subset_cells * 100)


                ncell_df[is.na(ncell_df)] <- 0

                if (sum(ncell_df$subset_cells) != ncell_df$all_cells[1]) {
                    stop("The number of cells doesn't match the sum of subsetted cells!")
                }

                return(ncell_df) }) %>%
            # Remove NULL elements
            compact()
        ncell_list[[celltype]][[counttype]] <- df_list
    }
}
```

Refer to the following count metrics:

* *subset_samples*: sample N
* *all_cells*: the number of cells in the current celltype
* *subset_cells*: the number of cells for the current study and disease conditions
* *CE_cells*: the number of cells (=barcodes) containing the given CEs
* *CE_counts*: the number of counts for the given CEs in the current subset
* *CEcounts_per_cell*: *CE_counts* / *subset_cells*
* *percent_CEcells*: (*CE_cells* / *subset_cells*) x 100

Excel-readable CSV files are linked below.

```{r display_ncell, results='asis'}

# --------------------------------------------------------------------------------
# This chunk presents the expression of genes with and without CEs
# --------------------------------------------------------------------------------

for (celltype in names(ncell_list)) {
    cat('##', celltype, '{.tabset}\n\n')
    for (counttype in names(ncell_list[[celltype]])) {
        cat('###', counttype, '\n\n')

        # Merge data frames
        df <- lapply(names(ncell_list[[celltype]][[counttype]]),
                     function(name) ncell_list[[celltype]][[counttype]][[name]] %>%
                         mutate(CE=name)) %>%
            bind_rows() %>%
            remove_rownames()
        df <- df[, c("CE", colnames(df)[-ncol(df)])]

        # subchunkify(paste0(celltype, "_", counttype))
        print(knitr::kable(df))
        cat('\n\n')

        csv <- file.path(out_dir,
                         paste0(celltype, "_", counttype, "_ncell.csv"))
