...
# Create the output files if missing
if os.listdir(r.annocode_dir).sort() != annotation_files.sort():
    cmd = [
        r.gtf2lc,
        "-o",
        r.annocode_path,
...
## Sashimi plots {.tabset}

Sashimi plots are displayed along with the mean usage of splicing junctions. Information
about individual introns in each intron cluster is provided in the ``introns`` table 
below the *Overview* section.

```{r print_sashimi, results='asis'}

# --------------------------------------------------------------------------------
# This chunk displays sashimi plots
# --------------------------------------------------------------------------------

for (name in names(py$rdata_dic)) {
    cat('###', name, '{.tabset}\n\n')
    load(py$rdata_dic[[name]])
    for (c in unique(cluster_ids)) {
        cat('####', c, '\n\n')
        p <- make_cluster_plot_ms(cluster_to_plot=c,
