...
mtx_path <- "../results_ExNeu2/junction_counts/thalamus_excitatory_perind_numers.counts.gz"

# Path to metadata table
meta_path <- "../../../input/thalamus_excitatory/combined_thalamus_metadata.csv"

# Path to Snakemake config
snakemake_config <- "../config/config_ExNeu2.yaml"
...
for (name in colnames(temp_df)[! colnames(temp_df) %in% c('intron', 'counts')]) {
    ...
    if (1 %in% dim(pca_input)) {
        cat('Input is 1-dimensional...')
    } else {
...
if not isinstance(r.contrast_cols, list):
    py_contrast_cols = [r.contrast_cols]
else:
    py_contrast_cols = r.contrast_cols

for c in py_contrast_cols:
    cmd = [
        r.lc_wrapper,
        "--num_threads 4",
        r.mtx_path,
