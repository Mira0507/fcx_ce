...
for (name in names(norm_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p_path <- file.path(
                out_dir,
                paste0(name, '_', ce, '_individual_junctions.pdf'))
...
for (name in names(norm_list)) {
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p_path <- file.path(
                out_dir,
                paste0(name, '_', ce, '_junction_categories.pdf'))
...
for (name in names(ratio_list)) {
    ...
    p <- ratio_plot(input_df=ce_df,
        ...
        ylim(min_y, max_y + max_y)

    print(p)

    p_path <- file.path(
        out_dir,
        paste0(name, '_ce_over_nonce.pdf'))
    ggsave(p_path, p, device="pdf")
    link_plot(p_path)
    cat('\n\n')
    }
...
for (name in names(ratio_list)) {
    ...
    p_path <- file.path(
        out_dir,
        paste0(name, '_ce_over_alljunction.pdf'))
    ggsave(p_path, p, device="pdf")
    link_plot(p_path)
    cat('\n\n')
    }
