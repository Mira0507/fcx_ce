...
- **Full** table (accessible via the Excel-readable files ending with `"*_full.csv"`):
    ...
    - `junction_counts`: number of junctions detected for the `exon_type` per sample (either raw or log-normalized)
...
$junction\_counts = sample\_cells \times \beta_1 + \beta_0$) were fitted to determine how significantly each metric 
(e.g. `junction_counts`) was affected by the per-sample cell number for each exon type. The `parameter` and `p_value`
columns represent $\beta_1$, the magnitude and direction of change, and the likelihood of rejecting
the null hypothesis, respectively. Additionally, the magnitude of correlation between two variables was summarized 
using an R-squared value, represented by the `R_squared` column.

Ideally, we'd like to use metrics that are unbiased by per-sample cell number when examining per-cell CE counts
across samples and disease conditions.

```{r ncell_validation, results='asis'}
...
for (celltype in names(ncount_list)) {
    ...
    for (counttype in names(ncount_list[[celltype]])) {
        ...
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            ...
            min_y <- min(input_df[['percent_detection']])
            max_y <- max(input_df[['percent_detection']])
