...
for (celltype in names(py$adata_dic)[1:2]) {
    ...
    for(counttype in c('junction_raw', 'junction_lognorm')) {
        ...
        gdf_list <- mclapply(junction_genes, function(gene) {
            ...
            # Specify a pseudocount for division to the non-zero minimum counts
            pseudo_c <- min(gene_df[gene_df$counts > 0,]$counts) * 0.1
            ...
            stat_df <- gene_df %>%
                ...
                mutate(junction_counts=ifelse(is.na(junction_counts), 0, junction_counts),
                       ...
                       percent_detection=round(junction_cells / sample_cells * 100, 3)) %>%
                as.data.frame()
            return(stat_df)
        }) %>%
...
for (celltype in names(ncount_list)) {
    ...
    for (counttype in names(ncount_list[[celltype]])) {
        ...
        for(gene in names(ncount_list[[celltype]][[counttype]])) {
            cat('#####', gene, '{.tabset}\n\n')
            df <- ncount_list[[celltype]][[counttype]][[gene]]
            summary_df <- df %>%
                group_by_at(vars(disease_col, exon_type)) %>%
                summarize(subset_cells=sum(sample_cells),
                          MEAN_junction_counts=mean(junction_counts),
                          MEDIAN_junction_counts=median(junction_counts),
                          MEAN_junction_cells=mean(junction_cells),
                          MEDIAN_junction_cells=median(junction_cells),
                          MEAN_percell_junctions=mean(percell_junctions),
                          MEDIAN_percell_junctions=median(percell_junctions),
                          MEAN_percent_detection=mean(percent_detection),
                          MEDIAN_percent_detection=median(percent_detection))

            name <- paste(celltype, counttype, gene, sep="_")
            cat('###### Summary\n\n')
            print(knitr::kable(summary_df))
            cat('\n\n')
            csv <- file.path(out_dir, paste0(name, "_summary.csv"))
            write.csv(summary_df, csv, row.names=FALSE, quote=FALSE)
            link_table(csv)
            cat('###### Full\n\n')
            subchunkify(name)
            csv <- file.path(out_dir, paste0(name, "_full.csv"))
