...
# Number of cells with and without cryptic junctions {.tabset}
...
- **Summary** table (accessible via the Excel-readable files ending with `"*_summary.csv"`):
    - `exon_type`: [annotation reference](#Annotating_input_junctions)
    - `subset_samples`: number of samples per disease condition per cell subtype
    - `subset_cells`: number of cells (barcodes) per per disease condition per cell subtype
    - `MEAN_junction_counts`: average number of junctions detected for the `exon_type` 
      per disease condition per sample
    - `MEDIAN_junction_counts`: median number of junctions detected for the `exon_type` 
      per disease condition per sample
    - `MEAN_junction_cells`: average number of cells with junction detection 
      associated with the `exon_type` per disease condition per sample
    - `MEDIAN_junction_cells`: median number of cells with junction detection 
      associated with the `exon_type` per disease condition per sample
    - `MEAN_percell_junctions`: average number of per-cell junctions associated with the `exon_type`
      per disease condition per sample
    - `MEDIAN_percell_junctions`: median number of per-cell junctions associated with the `exon_type`
      per disease condition per sample
    - `MEAN_percent_detection`: average junction detection rate associated with the `exon_type`
    - `MEDIAN_percent_detection`: median junction detection rate associated with the `exon_type`
...
- **Full** table (accessible via the Excel-readable files ending with `"*_full.csv"`):
    ...
    - `junction_cells`: number of cells with junction detection associated 
      with the `exon_type` per sample
    - `percell_junctions`: number of junctions detected for the `exon_type` per cell per sample
        ...
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            ...
            df_list <- split(input_df, input_df$exon_type) %>%
                mclapply(function(i_df) {
                    ...
                    metrics <- colnames(nsample)[str_detect(colnames(nsample), "MEDIAN|MEAN")]
