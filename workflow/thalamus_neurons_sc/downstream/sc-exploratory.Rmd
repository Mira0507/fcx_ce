...
for (celltype in names(ncount_list)) {
    ...
    for (counttype in names(ncount_list[[celltype]])) {
        ...
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            cat('\n\n####', paste0('CE: ', gene), '{.tabset}\n\n')
            # Prep a data frame for samples with and without CE detection
            input_df <- ncount_list[[celltype]][[counttype]][[gene]] %>%
                dplyr::filter(exon_type == 'cryptic') %>%
                mutate(CE=ifelse(junction_cells > 0, "detected", "undetected")) %>%
                group_by_at(vars(disease_col, 'CE')) %>%
                summarize(N=n()) %>%
                pivot_wider(names_from=CE, values_from=N) %>%
                mutate_at(
                    ce_colname,
                    ~ifelse(is.na(.x), 0, .x)) %>%
                pivot_longer(
                    cols=ends_with('detected'),
                    names_to="CE",
                    values_to="sample_N") %>%
                mutate(CE=factor(CE, levels=ce_colname))

            p <- ggplot(input_df,
                        aes_string(x=disease_col, y='CE', fill='CE')) +
...
for (celltype in names(ncount_list)) {
    ...
    for (counttype in names(ncount_list[[celltype]])) {
        ...
        for (gene in names(ncount_list[[celltype]][[counttype]])) {
            ...
            # Prep a data frame for each sample with and without CE detection
            input_df <- ncount_list[[celltype]][[counttype]][[gene]] %>%
                dplyr::filter(exon_type == 'cryptic') %>%
                mutate(CE=ifelse(junction_cells > 0, "detected", "undetected")) %>%
                group_by_at(vars(disease_col, 'CE')) %>%
