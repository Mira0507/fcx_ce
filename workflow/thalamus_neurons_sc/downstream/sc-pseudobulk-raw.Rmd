...
Raw pseudobulk counts are compared across disease conditions. Junctions 
...
raw_list <- mclapply(names(mat_list), function(name) {

    # Extract and clean the raw count matrix corresponding to the current subset
    df <- mat_list[[name]] %>%
...
for (name in names(raw_list)) {
    if (any(is.na(raw_list[[name]]))) {
        stop(paste0("Missing values introduced to the following contrast in `raw_list`:",
                    name))
    }
}
```
...
Raw pseudobulk counts are compared across the disease conditions. Junctions 
with zero counts across all samples are excluded.
...
Raw counts are aggregated on whether it results in CE generation.
    ...
    for (ce in ce_targets) {
        ...
        if (nrow(ce_df) > 0) {
            ...
            p <- count_plot(input_df=ce_df,
                            dcol=disease_col,
                            scol=study_col,
                            ylabel='Raw Counts for Junctions with or without CEs') +
